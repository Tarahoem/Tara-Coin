...
struct Params {
    ...
    /** Tara staking parameters */
    CAmount stakeMinimum{3 * COIN};
    CAmount gamingMinBet{COIN / 100};  // 0.01 TARA minimum bet
    int gamingTaxRate{50};  // 0.5% gaming tax (50 basis points = 0.5%)
    CAmount slashPenalty{3 * COIN};
    int64_t validatorSlotSeconds{86400};
    std::vector<std::string> genesisValidators;
    
    /** LP injection parameters */
    CAmount lpInjectionAmount{3000000 * COIN};
    int lpInjectionBlock{720};
    std::string ministryInjectionPubKey;
    
    /** Carbon credit fee: 0.01% = 10 sats per 100000 sats */
    CAmount carbonCreditFee{10};
    bool carbonCreditEnabled{true};
};
