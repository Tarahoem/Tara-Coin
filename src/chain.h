...
struct ValidatorEntry {
    ...
    int registeredAtHeight{0};
};

class ValidatorRegistry {
public:
    std::vector<ValidatorEntry> validators;
    uint32_t currentRoundIndex{0};
    std::map<uint256, std::string> slashLog;
    static const size_t MAX_SLASH_LOG = 10000;  // Prevent memory exhaustion
    std::map<uint256, int64_t> slashTimestamps;
    std::map<uint256, EscrowEntry> escrows;

    size_t Size() const { return validators.size(); }
    CPubKey GetAssignedValidator(uint32_t blockHeight) const {
        size_t count = validators.size();
        if (count == 0) return CPubKey();
        if (count > 1000000) return CPubKey();  // Overflow guard
        uint32_t index = blockHeight % static_cast<uint32_t>(count);
        if (index >= count) return CPubKey();  // Bounds check
        return validators[index].pubkey;
    }

    bool AddValidator(const CPubKey& pubkey, CAmount stake) {
        // Memory safety: prevent overflow
        if (validators.size() >= 1000000) return false;
        // Null/invalid check
        if (!pubkey.IsFullyValid()) return false;
        // Negative stake check
        if (stake <= 0) return false;
        // Format validation: pubkey must be correct size
        if (pubkey.size() != 33 && pubkey.size() != 65) return false;
        
        // Use-after-free prevention: verify iterator validity
        for (const auto& v : validators) {
            if (v.pubkey == pubkey) return false;  // Already registered
        }
        
        validators.push_back({pubkey, stake, GetTime(), 0});
        return true;
    }

        bool RemoveValidator(const CPubKey& pubkey) {
            if (!pubkey.IsFullyValid()) return false;  // Invalid input guard
            size_t before = validators.size();
            validators.erase(std::remove_if(validators.begin(), validators.end(),
                [&](const ValidatorEntry& v) { return v.pubkey == pubkey; }), validators.end());
            return validators.size() < before;  // True if actually removed
        }
};

extern ValidatorRegistry g_validatorRegistry;
extern CAmount g_slashedCoinsPending;
...
struct ProtocolTreasury {
    CAmount polBalance{0};
    CAmount feesEarned{0};
    CAmount lbeTotalDeposited{0};
    bool lbeActive{false};
    int64_t lbeEndTime{0};
    int64_t lbeStartTime{0};
    // API key tracking for commercial data access
    std::map<std::string, bool> apiKeys;  // key -> isCommercial
    std::map<std::string, CAmount> apiFeesCollected;
    CAmount totalApiFees{0};
    std::map<uint256, EscrowContract> escrows;
