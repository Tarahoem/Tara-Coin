...
static CPubKey GetBlockMinerPubKey(const CBlock& block)
{
    if (block.vtx.empty() || !block.vtx[0] || block.vtx[0]->vout.empty()) return CPubKey();
    const CScript& script = block.vtx[0]->vout[0].scriptPubKey;
    std::vector<unsigned char> data(script.begin(), script.end());
    if (data.size() == 35 && (data[0] == 0x21 || data[0] == 0x41)) {
        return CPubKey(data.begin() + 1, data.begin() + 34);
    }
    return CPubKey();
}
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // #9 Freelance escrow: 0.1% fee to LP (OP_ESCROW)
    // #10 API access: 1.5% commercial use (OP_API_ACCESS)
    // #11 Carbon credit: 0.01% fee (OP_CARBON)
    // #12 Shop payments: 0.5% fee, 50% to LP (OP_SHOP)
    // #13 Validator data: 0.00000001 TARA (OP_VALIDATOR_DATA)
    // #14 Token creation: fee to LP (OP_TOKEN)
    // #15 Merchant: fee to LP (OP_MERCHANT)
    for (const auto& tx : block.vtx) {
    ...
    // --- TARA FRAUD DETECTION ---
    if (g_validatorRegistry.Size() >= 3) {
        CPubKey assignedValidator = g_validatorRegistry.GetAssignedValidator(pindex->nHeight);
        CPubKey blockMiner = GetBlockMinerPubKey(block);
        if (!assignedValidator.IsFullyValid()) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "no-validator-assigned");
        }
        if (blockMiner != assignedValidator) {
            SlashValidator(blockMiner, block.GetHash(), "stolen-validator-slot");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "validator-mismatch");
        }
        bool hasStake = false;
        for (const auto& v : g_validatorRegistry.validators) {
            if (v.pubkey == blockMiner && v.stakeAmount >= 3 * COIN) {
                hasStake = true; break;
            }
        }
        if (!hasStake) {
            SlashValidator(blockMiner, block.GetHash(), "mining-without-stake");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "mining-requires-stake");
        }
    }
    ...
    // #2 3M TARA LP injection after 1 month (720 blocks)
    const Consensus::Params& consensus = params.GetConsensus();
    if (pindex->nHeight == consensus.lpInjectionBlock) {
        g_treasury.polBalance += consensus.lpInjectionAmount;
        g_treasury.lbeActive = true;
        LogInfo("LP INJECTION: %d TARA added to POL at block %d\n", consensus.lpInjectionAmount / COIN, pindex->nHeight);
    }
...
void SlashValidator(const CPubKey& validator, const uint256& txid, const std::string& reason)
{
    if (!validator.IsFullyValid() || g_validatorRegistry.Size() <= 3) return;
    g_validatorRegistry.slashLog[txid] = reason;
    g_validatorRegistry.RemoveValidator(validator);
    g_slashedCoinsPending += 3 * COIN;
    LogInfo("VALIDATOR SLASHED: 3 TARA -> LP, reason: %s\n", reason);
}
