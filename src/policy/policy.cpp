...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        // Token creation: OP_RETURN with TARA_TOKEN prefix
        } else if (whichType == TxoutType::NULL_DATA && txout.scriptPubKey.size() >= 14) {
            std::vector<unsigned char> scriptData(txout.scriptPubKey.begin(), txout.scriptPubKey.end());
            // Check for TARA_TOKEN marker (0x54 0x41 0x52 0x41 0x5f 0x54 0x4f 0x4b 0x45 0x4e)
            const char tokenMarker[] = "TARA_TOKEN";
            if (scriptData.size() >= 12 && scriptData[1] == 0x0a && memcmp(&scriptData[2], tokenMarker, 10) == 0) {
                // Token creation tx — allowed, 0.2% fee applied in miner
                continue;
            }
